...
#
# To read from ip webcam and write back out to disk:
# python people_counter.py --prototxt mobilenet_ssd/MobileNetSSD_deploy.prototxt \
#	--model mobilenet_ssd/MobileNetSSD_deploy.caffemodel --input rtsp://<ip-address>/MediaInput/h264\
	...
	for (objectID, centroid) in objects.items():
		...
		# to determine direction
			...
			if not to.counted:
				...
				if direction < -5 and 0 < aboveLinePos - centroid[1] and aboveLinePos - centroid[1] < 20:
					if arr[objectID] != 1:
						totalUp += 1
						to.counted = True
						arr[objectID] = 1
				...
				elif direction > 5 and 0 < centroid[1] - belowLinePos and centroid[1] - belowLinePos < 20:
					if arr[objectID] != 2:
						totalDown += 1
						to.counted = True
						arr[objectID] = 2

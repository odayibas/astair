...
public interface SensorRepository extends JpaRepository<Sensor, Long>, PagingAndSortingRepository<Sensor, Long> {
    ...
    @Query(nativeQuery = true, value = "select AVG(sensor_degree) as sensor_degree  from sensor s group by s.ac_id having s.ac_id = :ac_id")
    Float getSensorDegreeAve(Integer ac_id);

    @Query(nativeQuery = true, value = "select AVG(sensor_degree) as sensor_degree from sensor s")

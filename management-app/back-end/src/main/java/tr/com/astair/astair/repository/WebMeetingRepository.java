...
public interface WebMeetingRepository extends JpaRepository<WebMeeting, Long> {
    ...
    @Query(nativeQuery = true, value = "select * from meeting_web m where m.date >= :beginDate or m.date <= :finishDate")
    List<WebMeeting> getMeetingARange(@Param("beginDate") String beginDate, @Param("finishDate") String finishDate);
    @Query(nativeQuery = true, value = "select * from meeting_web m where m.room = :room and m.date >= :beginDate or m.date <= :finishDate")

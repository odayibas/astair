...
public interface WebMeetingRepository extends JpaRepository<WebMeeting, Long> {
    ...
    @Query(nativeQuery = true, value = "select to_date(cast(CURRENT_DATE + i as varchar), 'YYYY-MM-DD') " +
            "from generate_series(date(:firstDay) - CURRENT_DATE, date(:lastDay) - CURRENT_DATE ) i " +
            "where to_char(to_date(cast(CURRENT_DATE + i as varchar), 'YYYY-MM-DD'), 'DAY') != 'SATURDAY ' " +
                "and to_char(to_date(cast(CURRENT_DATE + i as varchar), 'YYYY-MM-DD'), 'DAY') != 'SUNDAY   ' " +
                "and to_date(cast(CURRENT_DATE + i as varchar), 'YYYY-MM-DD') >= current_date " +
            "except " +
            "select to_date(m.date, 'YYYY-MM-DD') " +
            "from meeting_web m " +
    ...
    List<String> appropriateDays(@Param("month") Integer month, @Param("interval") Double interval, @Param("firstDay") String firstDay, @Param("lastDay") String lastDay);
    ...
    @Query(nativeQuery = true, value = "select cast(concat(substring(cast(current_date as varchar), 1, 4), " +
            "'-', " +
            "cast(:month as varchar), " +
            "'-01') as varchar)")
    String findFirstDay(@Param("month") Integer month);

    @Query(nativeQuery = true, value = "select cast(cast(date(:firstDay) + interval '1 month -1 day' as date) as varchar)")
    String findLastDay(@Param("firstDay") String firstDay);

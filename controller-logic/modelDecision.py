...
def run_model(data, ac_id, model):
    ...
    if data[0] == {}:
        return -1
    if data[1] == {} or ac_id not in (data[1]):
        ...
        act = model.get_greedy_action(ac_data[1])
    ...
    action = model.forward(apparent_temp, survey_data)
...
def start():
    ...
    ac_list = {3: AC("Astair/+/+/#", "10.154.3.45")}
    ...
    while True:
        ...
        for id in ac_list:
            ac = ac_list[id]
            new_ac_conf = run_model(data, id, q_model)
            ac.set_temp(id, new_ac_conf)

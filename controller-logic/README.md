...
1) First of all, necessary data is fetched. It is grouped according to A/C id. (If there is more than one A/C) 
2) Then by using humidity and temperature, apparent temperature is calculated.
3) Apparant temperature is forwarded to SVM model and classified out of 5. 
4) Once the state is determined, the model looks the Q-table and choose the best action acording to the past experiences to maximize the future reward.
5) Then currently intended temperature is calculated by using the action. 
6) The new temperature value is sent to the A/C. 
7) After all this process, the users produce a feedback by voting. By using this feedback, the total reward is calculated. 
8) Then the relation (state, action) (mentioned above) is rewarded. Thereby, the Q-Model is trained.
